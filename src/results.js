...
export function showResult(outcome, snap){
  ...
  switch(outcome.type){
    ...
    case 'lowsteal':
      els.grade.classList.add('steal');
      els.grade.textContent = 'STOLEN ! 💀';
      els.desc.innerHTML = `Smite à ${pvBig(pv)}en dessous de 1000, trop bas. L'ennemi te l'a volé.`;
      break;

    case 'early':
      els.grade.classList.add('early');
      els.grade.textContent = 'TROP TÔT ⏱';
      els.desc.innerHTML = `Smite à ${pvBig(pv)}le Smite (1200) ne l'a pas tué (il en restait ${fr(snap - SMITE_DMG)}). Volé.`;
      break;

    default: // win
      els.grade.classList.add('win');
      els.grade.textContent = outcome.label;
      els.desc.innerHTML = `${outcome.note}<span class="pvbig">Smite à ${pv} <small>PV</small></span>à ${fr(SMITE_DMG - snap)} des 1200.`;
  }
}
